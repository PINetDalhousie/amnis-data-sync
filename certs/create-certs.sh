...
keytool -keystore server.keystore.jks -alias server -validity 365 -genkey -keyalg RSA -storepass $PASSWORD
...
keytool -keystore server.truststore.jks -alias CARoot -import -file ca-cert -storepass $PASSWORD
# Add the generated CA to the clients' truststore so that the clients can trust this CA
keytool -keystore client.truststore.jks -alias CARoot -import -file ca-cert -storepass $PASSWORD

# Step 3 - Sign the cert
keytool -keystore server.keystore.jks -alias server -certreq -file cert-file -storepass $PASSWORD
openssl x509 -req -CA ca-cert -CAkey ca-key -in cert-file -out cert-signed -days 365 -CAcreateserial -passin pass:$PASSWORD
keytool -keystore server.keystore.jks -alias CARoot -import -file ca-cert -storepass $PASSWORD
keytool -keystore server.keystore.jks -alias server -import -file cert-signed -storepass $PASSWORD
